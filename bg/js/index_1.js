...
ScrollTrigger.create({
  ...
  end: "+=2000", // 종료지점, 시작지점으로부터 4000px이후  
  pin: true, // 화면고정여부, true/false, 요소의 class/id 사용가능
  scrub: 5, // 스크롤 여부에 따라 애니메이션 되감기 기능, true/false, 숫자 사용가능
...
console.log($(".sec02").offset().top);
console.log($(".secstart").offset().top);
console.log($(".sec05").offset().top);

$(document).scroll(function () {
  var scrolltop = $(window).scrollTop();
  $(".sec01plus, .sec02, .secstart, .sec05").each(function () {
    if (scrolltop >= $(this).offset().top - 600) {
      $("li a[data-id=" + $(this).attr('class') + "]").addClass('on').parent().siblings().find("a").removeClass('on');
    }
  });
